...
import {Input, Textarea, Select, Button, useToast, useColorMode} from "@chakra-ui/react";
...
export function FormEditTodo({dataInit, onClose}) {
  const {
    control,
    formState: {errors},
    handleSubmit,
  ...
  const onSubmit = (todos) => {
    ...
    dispatch(
      genericAction(UPDATE_TODO, ENUM_STATUS.FETCHING, payload)
    );
  };
  useEffect(() => {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="p-10 border rounded-md">
      <Controller
        ...
        render={({field}) => (
          <Input placeholder="Title" color={colorMode === 'light' ? 'black' : 'white'} f {...field} />
        )}
      />
