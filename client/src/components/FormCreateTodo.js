...
import {Input, Textarea, Button, useToast, Code, useColorMode} from "@chakra-ui/react";
...
export function FormCreateTodo() {
  ...
  const onSubmit = (data) => {
    dispatch(genericAction(CREATE_TODO, ENUM_STATUS.FETCHING, {data, statusType}));
  };
  ...
  return (
    ...
    >
      <Controller
        ...
        rules={{required: true}}
        render={({field}) => (
          <Input placeholder="Title" color={colorMode === 'light' ? 'black' : 'white'} {...field} />
        )}
      />
